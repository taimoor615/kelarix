<?php
/**
 * Industries: Our Value (split + 7-item accordion, first item open with dismiss X).
 *
 * @package Kelarix
 */
$icons = array(
...
?>
<section class="section section--accent industries-value discipline">
	<div class="container discipline__inner">
		<div class="discipline__aside">
			<span class="badge badge--dashed"><span class="dot"></span><?php k_text( 'value_badge', 'Our Value' ); ?></span>
			<h2 class="section__title section__title--light">
				<?php k_html( 'value_heading', 'Built with the discipline sensitive business systems require.' ); ?>
			</h2>
			<p class="section__lead section__lead--light">
				<?php k_text( 'value_text', 'Kelarix works with systems that affect reporting, workflows, operations, customer processes, internal visibility, and decision making. That requires clarity, confidentiality, documentation, governance, and responsible automation.' ); ?>
			</p>
		</div>

		<div class="discipline__list" data-accordion>
			<?php for ( $i = 1; $i <= 7; $i++ ) :
				$item    = k_field( 'value_item_' . $i, array() );
